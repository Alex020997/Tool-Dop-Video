...
Bước 2: Thêm kênh YouTube cần theo dõi
   → Nhập Channel ID "Channel ID/URL" → Ví dụ Channel ID: UCiz19rgacvujLo0-FH8u5xA
   → Click "Đăng ký" (Nếu nhiều channel thì cách nhau bởi dấu , 
...
Bước 5: Dừng theo dõi
   ...
   → Hoặc chọn kênh cần hủy, sau  click "Hủy Đăng Ký Kênh Nhận Thông Báo" để xóa kênh khỏi danh sách
...
Lỗi: "Invalid channel ID"
   ...
   → Đảm bảo Channel ID bắt đầu bằng "UC" và có 24 ký tự

Lỗi: "Failed to start tunnel"
   → Kiểm tra Ngrok Token có đúng không
   → Kiểm tra kết nối Internet
   → Thử restart tool

Lỗi: Không nhận được thông báo Telegram
   → Kiểm tra Telegram Token và ID có đúng không
   → Đảm bảo đã gửi lệnh /start cho bot của bạn
